...
int remove_student_from_student_file_by_no(int std_no) {
	...
	while(fgets(block,LINESIZE, fp) != NULL ) {
		...
		if( !strcmp(std_no_from_file, cast(std_no)) ) {
			...
			char* unknown = create_unknown(5);
			fputs(unknown,fp);
			free(unknown);free(block);
			return SUCCESS;
		}
	}
	return FAIL;
...
int main() {
	...
	FILE* fp ;
	fp = fopen("set_cursor_previous_line_begin.txt", "r+");
	char* str = (char*)malloc(LINESIZE);
	fgets(str,LINESIZE,fp);
	fgets(str,LINESIZE,fp);
	set_cursor_previous_line_begin(fp);
	printf("iki satır okudum önceki satıra dönmek istiyorum önceki satır 21.satır\n");
	printf("ftell: %ld",ftell(fp));
	printf("basına x yazalım.");
	fputs("x",fp);
