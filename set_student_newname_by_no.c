...
int set_student_newname_by_no(char* newname, int no_){
	...
	char* block = (char*) malloc(LINESIZE);
	while(fgets(block, LINESIZE, fp)) {
		char* std_name = get_unique_element(block,student_std_name);
		int line_off =  strlen(std_name) - strlen(newname);
		char* std_no = get_unique_element(block, student_std_no);
		printf("\n%d", line_off);
		//c_code[line_off+strlen(block)] = '\0';
		if( !strcmp(no , std_no) ) {
			char *n_block = set_unique_element(block, newname, student_std_name);
			if(line_off > 0) {
				set_cursor_previous_line_begin(fp);
				write_space_line(fp);
				set_cursor_previous_line_begin(fp);
				fputs(n_block, fp); free(block); free(std_name) ;fclose(fp);
			} else {
				recreation_of_lecture_file(fp, std_no, n_block);
			}
			free(n_block);
			return SUCCESS;
		}
		free(std_name);free(std_no);
	}
	free(block);fclose(fp);
	return FAIL;
}
...
int main() {
	...
	set_student_newname_by_no("ali",18011078);
