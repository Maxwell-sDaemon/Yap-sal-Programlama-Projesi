...
void get_only_teacher_block_by_id(char* t_id) {
	FILE *fp;
	fp = open(TEACHERFILE, "r");
	char* block = (char*) malloc(LINESIZE);
	while( fgets(block, LINESIZE, fp ) ) {
...
int move_log_to_end(char* l_id, char* l_block) {
	...
	if(fputs(n_block, fp) != EOF) {
		free(block);free(n_block);fclose(fp);free(value);free(time);
		return SUCCESS;
	}
	free(time);
...
int is_student_add_lec_available(char* st_no, char* given_course, int max_credit, int max_lec ) {
	...
	while( fgets(block, LINESIZE, fp) ) {
		...
		//printf("%s\t",std_no_log);
		//printf("%s\t",course_name_log);
		//printf("%s\t",given_course);
		//printf("%s\t",reg_stat_log);
		//printf("%s\t",given_course);
		//printf("%s\t",st_no);
...
int create_file_for_class_by_course_code(char* c_code) {
	...
	while( fgets(block, LINESIZE, fr) ) {
		...
		char *s_block = get_student_block_by_student_no(s_no);
		
		//STUDENT student = (STUDENT*) malloc(sizeof(STUDENT));
		
		//sscanf(s_block ,"%s" SEP "%s" SEP "%s" SEP , student.no, student.name, student.surname );
		////printf("%d : %s\t%s\t%s\n " list_id, student.no, student.name, student.surname);
		//int end_ptr = strlen(student.no) + strlen(student.name) + strlen(student.surname);
		//s_block[end_ptr] = '\0';
...
int main() {
	...
	//sscanf("18011078,onur,demir,9,21" ,"%d" SEP "%s" SEP "%s" SEP , &student.stdno, student.name, student.surname );
