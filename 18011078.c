...
int set_student_newsurname_by_no(char* newsurname, int no_){
	...
	while(fgets(block, LINESIZE, fp)) {
		char *block_no = get_unique_element(block,student_std_no);
		//char *stdname = get_unique_element(block,student_std_surname); 
		if( !strcmp(block_no,no)) {
			//int len_b = strlen(block);
			//int len = strlen(newsurname) - strlen(stdname) ;
...
int main() {
	...
	//set_student_newname_by_no("onur",10011000);
	
	//set_student_newsurname_by_no("aklem",15070026);
