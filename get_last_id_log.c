...
int add_student_to_lecture_byno(char* student_no, char* course_code, int max_credit, int max_lec) {
	FILE* fp;
...
int move_log_to_end(char* l_id, char* log_block) {
	FILE *fp; 
	invert_log_from_id(l_id);
	fp = fopen(LOGFILE, "a");
	int id = get_last_id_log();
	
	char* value = integer_to_string(id,PARTSIZE);
	char* block = set_unique_element(log_block, value, log_id);
...
// remove_student_from_lecture_byno(course_name, std_no)
...
int main() {
	...
	printf("last id in the log is %d", id);
