...
void get_course_block_by_course_code(char *course_code){
	FILE *fp;
	fp = fopen(LECFILE, "r");
	char* block = (char*)malloc(LINESIZE);
	while(fgets(block, LINESIZE, fp)) {
		...
		if(! strcmp(c_code ,course_code)) {
			...
			free(c_code);free(block);
			return;
		}
		free(c_code);
	}
	printf("ilgili ders bulunamadı.");
	free(block);
}
...
TEACHER* create_teacher() {
	...
	for( i = 0 ; unvanlar[option][i] != '\0' ; i++) {
		teacher->degree[i] = unvanlar[option][i];
	}
	teacher->degree[i] = '\0';
	return teacher;
}
